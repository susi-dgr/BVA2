...
def richardson_lucy(B, K, iterations=30, init_img=None):
    ...
    for i in range(iterations):
        ...
        conv = cv2.filter2D(A_est, -1, K)
        ratio = B / (conv + 1e-6)
        correction = cv2.filter2D(ratio, -1, K_mirror)
        A_est *= correction
...
def get_kernels(kernel_size):
    return {
        ...
        "gaussian": cv2.getGaussianKernel(kernel_size, kernel_size / 3) @ cv2.getGaussianKernel(kernel_size, kernel_size / 3).T,
...
def test_rld_on_image(img_path):
    ...
    for name, K in kernels.items():
        print(f"\n\nTesting kernel: {name}")
        blurred = cv2.filter2D(original, -1, K)
